...
- Nombre de usuario del *retweet*: Se obtuvo a partir del *tweet* extrayendo el usuario mencionado cuando existía "RT @" en el *tweet*.
...
- Remover:
  - Acentos
  - Caracteres especiales como "’"?¡!*,.():;-_¿", a
...
Con el objeto anterior, formamos los siguientes histrograma. El de lado izquierdo nos muestra que tenemos muchas conexiones muy débiles donde la frecuencia de la arista es de 1, 2 o 3. El gráfico de la derecha, está filtrado con aristas mayores a 5. En este se puede apreciar que seguimos teniendo muchas conexiones débiles pero no tantas como si filtramos para conexiones arriba de 10. 
...
Con el siguiente nodo obtenemos los nodos y volvemos a armar la red con frecuencia de conexión arriba de 10. 
...
Y obtenemos la siguiente figura es una representación de la red que muestra las conecciones entre los nodos. En esta se observa una agrupación principal alrededor del usuario `DesSocial_CDMX`, que pareciera ser el nodo que tiene más *grado* pues tiene muchas aristas conectadas hacia él. Esta cuenta, correspondía a la Secretaría de Desarrollo Social de la Ciudad de México de ese periodo. También, se observan algunas agrupaciones alejadas y no conectadas a la agrupación principal. 
...
La siguiente figura presenta la red tomando en cuenta la medida de **intermediación**, que recordemos que nos proporciona una medida indicando qué tan único o importante es un nodo para conectar con otros nodos en la red. Para este caso resalta mucho el usuario de `ManceraMiguelMX` y `GobCDMX`, que parece contienen conexiones únicas y en el caso del primero parece haber una conexión sobre todo para la cuentas oficiales como lo son el `C5_CDMX` y noticieros también oficiales. 
...
En el siguiente gráfico observamos que la medida de centralidad de eigenvector tanto en transparencia del texto de usuario, el tamaño y el color del punto. Lo que observamos es que efectivamente toma en cuenta estar cercano a nodos importantes y a su vez conectarse a muchos, que el usuario más relevante para nuestro conjunto de datos en menciones es `DesSocial_CDMX`, pues tiene el valor más alto, mientras que los otros usuarios tienen menos de 0.25. 

```{r, echo=FALSE, fig.dim=c(8,6)}
usi %>%
  activate(nodes) %>% 
  ggraph(layout = 'graphopt', spring.constant = 0.25, charge = 0.05, niter = 300) + 
  geom_edge_link2(arrow = arrow(length = unit(2, 'mm')), alpha = 0.05, colour="black") + 
...
La siguiente figura es una representación de la red que muestra las conecciones entre los nodos. Se puede observar más agrupaciones entre los nodos, una de estas agrupaciones parece centrarse alrededor de `ManceraMiguelCDMX` y algunas cuentas oficiales del mismo gobierno. También, obseramos otra agrupación con el grupo `Milenio`, y otras tantas agrupaciones con varios nodos.  
...
Para analizar con más detalle esta red observamos la siguiente figura que es con la medida de *intermedicación*, que parece cambiar completamente la estructura de la red destacando `GobCDMX` y `seduviCDMX`. La cuenta de `ManceraMiguelCDMX` parece mantenerse en el centro de la red con muchas conexiones. 
...
También, observamos que cambian mucho las redes dependiendo de la medida que utilices, si bien el número de conexiones parecen ser importantes, también lo es indagar más en las redes para ver si esos usuarios también tienen importancia siendo usuarios únicos. 
